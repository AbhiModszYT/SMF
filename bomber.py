...
.----------------. .----------------. .----------------. .----------------. .----------------. 
| .--------------. | .--------------. | .--------------. | .--------------. | .--------------. |
| |      __      | | | ____    ____ | | |   ______     | | |     ____     | | |  _________   | |
| |     /  \     | | ||_   \  /   _|| | |  |_   _ \    | | |   .'    `.   | | | |  _   _  |  | |
| |    / /\ \    | | |  |   \/   |  | | |    | |_) |   | | |  /  .--.  \  | | | |_/ | | \_|  | |
| |   / ____ \   | | |  | |\  /| |  | | |    |  __'.   | | |  | |    | |  | | |     | |      | |
...
def do_zip_update():
    success = False
    if DEBUG_MODE:
        zip_url = "https://github.com/AbhiModszYT/SMF/archive/main.zip"
        dir_name = "SMF-main"
    else:
        zip_url = "https://github.com/AbhiModszYT/SMF/archive/main.zip"
        dir_name = "SMF-main"
    ...
    if success:
        mesgdcrt.SuccessMessage("AMBOT was updated to the latest version")
        mesgdcrt.GeneralMessage(
            "Please run the script again to load the latest version")
    else:
        mesgdcrt.FailureMessage("Unable to update AMBOT.")
        mesgdcrt.WarningMessage(
            "Grab The Latest one From https://github.com/AbhiModszYT/SMF.git")
...
def do_git_update():
    ...
    if success:
        ...
        print(
            "git checkout . && "
            "git pull https://github.com/AbhiModszYT/SMF.git HEAD")
    sys.exit()
...
def check_for_updates():
    ...
    if fver != __VERSION__:
        ...
        mesgdcrt.SuccessMessage("AMBOOM is up-to-date")
        mesgdcrt.GeneralMessage("Starting AMBOOM")


def notifyen():
    try:
        if DEBUG_MODE:
            url = "https://github.com/AbhiModszYT/SMF/raw/main/.notify"
        else:
            url = "https://github.com/AbhiModszYT/SMF/raw/main/.notify"
...
def pretty_print(cc, target, success, failed):
    ...
    mesgdcrt.SuccessMessage("AMBOOM was created by AMBOT")
...
if sys.version_info[0] != 3:
    mesgdcrt.FailureMessage("AMBOOM will work only in Python v3")
    sys.exit()
...
__CONTRIBUTORS__ = ['AMBOT', 'AbhiModszYT']
...
description = """AMBOOM - Your Friendly Spammer Application

AMBOOM can be used for many purposes which incudes -
\t Exposing the vulnerable APIs over Internet
\t Friendly Spamming
\t Testing Your Spam Detector and more ....

AMBOOM is not intented for malicious uses.
"""

parser = argparse.ArgumentParser(description=description,
                                 epilog='Coded by AbhiModszYT !!!')
parser.add_argument("-sms", "--sms", action="store_true",
                    help="start AMBOOM with SMS Bomb mode")
parser.add_argument("-call", "--call", action="store_true",
                    help="start AMBOOM with CALL Bomb mode")
parser.add_argument("-mail", "--mail", action="store_true",
                    help="start AMBOOM with MAIL Bomb mode")
parser.add_argument("-ascii", "--ascii", action="store_true",
                    help="show only characters of standard ASCII set")
parser.add_argument("-u", "--update", action="store_true",
                    help="update AMBOOM")
parser.add_argument("-c", "--contributors", action="store_true",
                    help="show current AMBOOM contributors")
parser.add_argument("-v", "--version", action="store_true",
                    help="show current AMBOOM version")
